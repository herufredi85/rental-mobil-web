...
class C_Detailtransaksi extends Controller {
	...
	public function indextgl($tglstart,$tglend){
		...
		$data = [
            'aktif' => 'detailtransaksi',
			'judul' => 'Detail Transaksi Uang Masuk dan Uang Keluar',
			'tglstart' => $tglstart,
			'tglend' => $tglend,
			'data_perjalanan' => $this->perjalanan->lihattgldetail($tglstart,$tglend),
			'no' => 1
		];
		$this->view('detailtransaksi/index', $data);
	}
	...
	public function xls($tglstart,$tglend){
		...
		$this->view('detailtransaksi/xlscomposer', $data);
	}
